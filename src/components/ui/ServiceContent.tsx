...
import { FaCheck, FaGlobe, FaFacebook, FaRobot, FaBullhorn } from "react-icons/fa6";
...
interface ServiceContentProps {
  services: (Service & { features: string[], benefits: string[] })[];
  selectedSlug?: string;
}

export default function ServiceContent({ services, selectedSlug }: ServiceContentProps) {
  const t = useTranslations("ServicesPage");
  const { translateServices } = useTranslateDbContent();

  // Translate services for display
  const translatedServices = translateServices(services) as (Service & { features: string[], benefits: string[] })[];

  // Find selected service from original (for slug matching) but display translated
  const selectedServiceIndex = selectedSlug
    ? services.findIndex((s) => s.slug === selectedSlug)
    : 0;
  const selectedService = selectedServiceIndex >= 0 ? translatedServices[selectedServiceIndex] : translatedServices[0];
        ...
        <div className="bg-gradient-to-br from-slate-700/90 via-slate-800/95 to-slate-900/90 backdrop-blur-xl border border-slate-600/30 rounded-3xl p-6 sticky top-24 shadow-2xl shadow-slate-900/50">
          ...
          <div className="space-y-1">
            {translatedServices.map((service, idx) => (
              <Link
                key={service.id}
                href={`/services?slug=${services[idx].slug}`}
                className={`block px-4 py-3.5 rounded-xl transition-all duration-300 ${services[selectedServiceIndex]?.id === service.id
        ...
        {selectedService ? (
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl p-8 md:p-10 shadow-2xl shadow-slate-200/50 border border-slate-100">
              ...
              <div>
                ...
                <ul className="space-y-4">
                  ...
                  {selectedService.features.length === 0 && (
                    <li className="text-slate-400 italic">{t("noFeatures")}</li>
                  )}
                </ul>
              </div>
              ...
              <div>
                ...
                <ul className="space-y-4">
                  ...
                  {selectedService.benefits.length === 0 && (
                    <li className="text-slate-400 italic">{t("noBenefits")}</li>
                  )}
                </ul>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-4 pt-4">
              <Button href="/#contact">{t("orderBtn")}</Button>
              <Link href="/#pricing">
                <Button variant="outline">{t("pricingBtn")}</Button>
              </Link>
            </div>
          </div>
        ) : (
