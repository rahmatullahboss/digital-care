...
document.addEventListener('DOMContentLoaded', () => {
    const styleId = 'floating-contact-buttons-style';
    ...
    const isBangla = htmlLang.startsWith('bn');
    ...
    const callText = withFallback(bodyDataset.callButtonLabel, isBangla ? 'কল করুন' : 'Call Us');
    const messageText = withFallback(bodyDataset.messageButtonLabel, isBangla ? 'মেসেজ করুন' : 'Message Us');
    const callHref = withFallback(bodyDataset.callButtonHref, 'tel:01639590392');
    const messageHref = withFallback(bodyDataset.messageButtonHref, 'https://www.m.me/profile.php?id=61579772421831');

    let floatingWrapper = document.querySelector('.floating-contact-buttons');
    if (!floatingWrapper) {
    ...
    const createButton = (className, href, iconClass, text, openInNewTab = false) => {
    ...
    const callButton = createButton('floating-call', callHref, 'fa-solid fa-phone', callText);
    const shouldOpenInNewTab = /^(https?:)?\/\//i.test(messageHref);
    const messageButton = createButton('floating-message', messageHref, 'fa-brands fa-facebook-messenger', messageText, shouldOpenInNewTab);

    floatingWrapper.append(callButton, messageButton);
});
